<?php ob_start()  ?>


<!DOCTYPE html>
<html lang="en">

<head>

    <meta charset="utf-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1">          
    <meta name="keywords" content="best school in yamunanagar,Top School In Yamunanagar,schools in yamunanagar haryana,top schools in yamunanagar,best school in yamunanagar jagadhri,top 10 schools in yamunanagar,best school in haryana,top cbse school in haryana,best sports school in haryana,cbse  school in haryana,top 10 cbse schools in haryana,best school in yamunanagar,best cbse school in yamunanagar,cbse school in yamunanagar">
    <meta name="description" content="Swami Vivekanand Public School is one of the Best Cbse school in Yamunanagar. It is best school in haryana.">
    <meta property="og:title" content="Best CBSE School In Yamunanagar-Haryana:- SVPS">
    <meta property="og:site_name" content="svps17huda.com">
    <meta property="og:url" content="https://svps17huda.com/index.php">
    <meta property="og:description" content="Swami Vivekanand Public School is one of the Best Cbse school in Yamunanagar. It is best school in haryana offering world class campus and infrastructure">
    <meta property="og:type" content="website">
    <meta property="og:image" content="https://svps17huda.com/assets/svps-logo/SVPS-original-white.png"> 

    <!-- CSS Plugins -->
    <link href="assets/plugins/lightbox/dist/css/lightbox.css" rel="stylesheet">
    <link href="assets/plugins/ionicons/css/ionicons.min.css" rel="stylesheet" type="text/css">
    <link rel="stylesheet" href="assets/plugins/owl-carousel/dist/assets/owl.carousel.min.css">
    <link rel="stylesheet" href="assets/plugins/owl-carousel/dist/assets/owl.theme.default.min.css">
    <link rel="shortcut icon" href="assets/svps-logo/logo.png">

    

    <!--Custom CSS-->
    <link rel="stylesheet" href="assets/css/style4.css">
    <link rel="stylesheet" href="extra/css/responsive.css">

    <!-- font style -->
    <!-- Google Tag Manager -->
    <script>(function(w,d,s,l,i){w[l]=w[l]||[];w[l].push({'gtm.start':
    new Date().getTime(),event:'gtm.js'});var f=d.getElementsByTagName(s)[0],
    j=d.createElement(s),dl=l!='dataLayer'?'&l='+l:'';j.async=true;j.src=
    'https://www.googletagmanager.com/gtm.js?id='+i+dl;f.parentNode.insertBefore(j,f);
    })(window,document,'script','dataLayer','GTM-WJBH27R');</script>
    <!-- End Google Tag Manager -->    

    <!-- Global site tag (gtag.js) - Google Analytics -->
    <script async src="https://www.googletagmanager.com/gtag/js?id=UA-202846356-1"></script>
    <script>
      window.dataLayer = window.dataLayer || [];
      function gtag(){dataLayer.push(arguments);}
      gtag('js', new Date());

      gtag('config', 'UA-202846356-1');
    </script>


    <script type="application/ld+json">
    {
      "@context": "https://schema.org",
      "@type": "LocalBusiness",
      "name": "Swami Vivekanand Public School",
      "image": "https://svps17huda.com/assets/svps-logo/SVPS-original-white.png",
      "@id": "",
      "url": "https://svps17huda.com/",
      "telephone": "01732-220158",
      "priceRange": "555",
      "address": {
        "@type": "PostalAddress",
        "streetAddress": "Sector-17, HUDA,",
        "addressLocality": "Yamunanagar",
        "postalCode": "135003",
        "addressCountry": "IN"
      },
      "geo": {
        "@type": "GeoCoordinates",
        "latitude": 30.1522754,
        "longitude": 77.30085059999999
      } ,
      "sameAs": [
        "https://www.facebook.com/svps.yamunanagar/",
        "https://twitter.com/SVPShuda",
        "https://www.instagram.com/svpshuda/",
        "https://www.youtube.com/channel/UCS8mpnKYjRR2RJxXkC2tACg"
      ]
    }
    </script>

    <title>Best CBSE School In Yamunanagar-Haryana:- SVPS </title>




</head> 

    <!-- Global site tag (gtag.js) - Google Analytics -->
    <!-- <script async src="https://www.googletagmanager.com/gtag/js?id=UA-157022361-3"></script> -->
    <!-- <script>
    window.dataLayer = window.dataLayer || [];
    function gtag(){dataLayer.push(arguments);}
    gtag('js', new Date());

    gtag('config', 'UA-157022361-3');
    </script> -->
    
    <!--<link rel="stylesheet" href="assets/plugins/aos-master/dist/aos.css">-->
    
    

    <!-- CSS Global -->
    <!--build:css assets/css/theme.min.css-->
    <!--<link rel="stylesheet" href="assets/css/theme.css">-->
    <!--endbuild-->
    <!-- <link rel="stylesheet" href="extra/style.css"> -->
    
    







